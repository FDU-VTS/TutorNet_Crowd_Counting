...
def make_layer_18(block, planes, blocks, stride=1):
    downsample = None
    inplanes = 128
    if stride != 1 or inplanes != planes * block.expansion:
        downsample = nn.Sequential(
...
class Baseline_resnet(nn.Module):
    def __init__(self, upscale_factor=16):
        super(Baseline_resnet, self).__init__()
        self.base = resnet18()
        self.own_rese_layer3 = make_layer_18(BasicBlock, 256, 2, stride=1)
        self.own_rese_layer3.load_state_dict(self.base.layer3.state_dict())
        self.base.layer3 = nn.Sequential()
        self.base.layer4 = nn.Sequential()

        self.output_layer = nn.Sequential(nn.Conv2d(256, 1, kernel_size=1), nn.ReLU(inplace=True))
...
class Baseline_densenet(nn.Module):
    def __init__(self, upscale_factor=16):
        super(Baseline_densenet, self).__init__()
        self.base = densenet121()
        self.base.features.transition3.pool = nn.Sequential()  # stride

        self.output_layer = nn.Sequential(nn.Conv2d(1024, 512, kernel_size=1),
                                          nn.ReLU(),
                                          nn.Conv2d(512, 1, kernel_size=1))
        self.gap = nn.AdaptiveAvgPool2d(1)
        self.up = torch.nn.Upsample(scale_factor=16)
        self.pixel_shuffle = nn.PixelShuffle(upscale_factor)
