...
from networks.baseline_de import Baseline_densenet
...
def main():
    ...
    dataloader_test = torch.utils.data.DataLoader(dataset_test, batch_size=1, shuffle=False, num_workers=20)
        ...
        for phase in ['train', 'test']:
            ...
            if phase == 'train':
                ...
                for rgb, ground_truth, _ in dataloader_test:
                    ...
                    print("This Step Used", time.time() - step_time)
                print("MAE", MAE.item() / dataset_test.__len__())
                print("MSE", math.sqrt(MSE.item() / dataset_test.__len__()))
                writer.add_scalar('scalar/MAE', MAE.item() / dataset_test.__len__(), epoch)
                ...
                if sum_MAE < sumMAE_best:
                    ...
                    sumMAE_best = sum_MAE
                    torch.save(best_model_wts, "Dense+res50_shanghai_" + "MAE" + str(sum_MAE)
                               + 'MSE' + str(
                        math.sqrt(MSE.item() / dataset_test.__len__() * batch_size)) + 'best_model_wts.pkl')

                    best_model_wts = copy.deepcopy(lossnet.state_dict())
                    torch.save(best_model_wts, "lossnet_shanghai_" + "MAE" + str(sum_MAE)
                               + 'MSE' + str(
                        math.sqrt(MSE.item() / dataset_test.__len__())) + 'best_model_wts.pkl')
