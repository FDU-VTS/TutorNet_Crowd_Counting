...
from networks.MCNN import MCNN
        ...
        for phase in ['train', 'test']:
            ...
            if phase == 'train':
                ...
                if sum_MAE < sumMAE_best:
                    ...
                    torch.save(best_model_wts, "tutor_mcnn_shanghai_" + "MAE" + str(sum_MAE)
                               + 'MSE' + str(
                        math.sqrt(MSE.item() / dataset_test.__len__() * batch_size)) + 'best_model_wts.pkl')

                    best_model_wts = copy.deepcopy(lossnet.state_dict())
                    sumMAE_best = sum_MAE
                    torch.save(best_model_wts, "for_lossnet_shanghai_" + "MAE" + str(sum_MAE)
                               + 'MSE' + str(
                        math.sqrt(MSE.item() / dataset_test.__len__() * batch_size)) + 'best_model_wts.pkl')
